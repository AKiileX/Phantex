...
// Package tls provides TLS configuration helpers for the Phantex gateway.
//
// Supports mTLS with automatic certificate reloading (no restart needed).
// In production, certs come from Vault PKI with 24h TTL.
package tls
...
type Config struct {
	...
	// CA certificate for verifying client certs (mTLS)
	CAFile string
	...
	// When set, the root CA in the peer's chain must match this fingerprint.
...
func (p *Provider) loadCertificates() error {
	...
	p.log.Info("tls_certificates_loaded",
		...
		zap.String("key", p.cfg.KeyFile),
		zap.String("ca", p.cfg.CAFile))

	return nil
}

// ServerTLSConfig returns a *tls.Config for a gRPC/HTTP server with mTLS.
func (p *Provider) ServerTLSConfig() *tls.Config {
	...
	cfg := &tls.Config{
		...
		// Use GetCertificate for hot-reload support
	...
	if p.cfg.RequireClientCert && p.caPool != nil {
		cfg.ClientAuth = tls.RequireAndVerifyClientCert
		// Set ClientCAs as baseline; VerifyPeerCertificate callback
		// re-reads p.caPool under lock for hot-reloadable CA trust.
		cfg.ClientCAs = p.caPool
		cfg.VerifyPeerCertificate = p.verifyClientCert
	}

	return cfg
}

// verifyClientCert re-verifies the client certificate chain against the
// current (possibly hot-reloaded) CA pool. This ensures CA rotation takes
// effect without restarting the server.
func (p *Provider) verifyClientCert(rawCerts [][]byte, _ [][]*x509.Certificate) error {
	if len(rawCerts) == 0 {
	...
	// fingerprint matches the pinned value. This prevents CA impersonation
	// even if an attacker obtains a rogue CA trusted by the system.
...
// matches the expected SHA-256 fingerprint (hex-encoded, colon-separated or
// plain hex accepted).
...
// ClientTLSConfig returns a *tls.Config for a gRPC/HTTP client with mTLS.
func (p *Provider) ClientTLSConfig() *tls.Config {
	...
	cfg := &tls.Config{
		MinVersion: p.cfg.MinVersion,
		// Use GetClientCertificate for hot-reload support
...
// certificates from disk. Call Stop() to terminate.
